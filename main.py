...
def main():
    ...
    # 解析cmd參數
    # =========================
    parser = argparse.ArgumentParser(description="Generate aesthetic QR code",
                                     epilog="Hope you enjoy!")
    parser.add_argument("v", help="Version, must be 1~40",
                        type=int)
    ...
    parser.add_argument("--mask", help="Mask use in generating QR code, must be 0~7",
                        default=0,
                        type=int)

    parser.add_argument("-ms", "--module_size", help="Determine how many pixels in a module",
                        default=13,
                        type=int)
    parser.add_argument("-s", "--subsize", help="Determine subsize of single module",
                        default=3,
                        type=int)
    ...
    binary_img_source, old_threshold, variance = Process.OTSU(
        source.img_luminance)
    ...
    Ideal_qr = Process.module_based_blending(
        base_qrcode, binary_image, qrcanvas.m)
    ...
    # ------------------------------------------------------------------------------------------------------------------------------------------------- #
    Process.select_module(qrcanvas, qrargs, qrdata,
                          narrow_blendmask, base_qrcode, binary_image)
    ...
    for subsize in range(3, 14, 2):
        for quality_factor in range(0, 51, 5):
            ...
            blending_qr = Process.blending_H(
                qrcanvas, XOR_qrcode, preprocessed_img, preprocessed_binary, new_threshold, subsize)
